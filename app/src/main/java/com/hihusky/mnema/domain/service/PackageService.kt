        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (!fileName.lowercase().endsWith(".zip") &&
                    ...
                    !fileName.lowercase().endsWith(".mnemapkg")
                ) {
                    return@withContext ImportResult.Error("Invalid file type. Please select a .zip, .quizpkg, or .mnemapkg file.")
                }
                ...
                tempPackage = File.createTempFile("import-", ".mnemapkg", context.cacheDir)
                ...
                val error = dbRepository.importData(plainMap, packageId) { importProgress ->
    ...
    suspend fun importBuiltInPackage(assetPath: String): ImportResult {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                tempPackage = File.createTempFile("builtin-import-", ".mnemapkg", context.cacheDir)
                ...
                val plainMap = jsonElementMapToPlain(dataMap)
                val error = dbRepository.importData(plainMap, packageId)
                if (error != null) {
                    destDir.deleteRecursively()
                    return@withContext ImportResult.Error(error)
                }
