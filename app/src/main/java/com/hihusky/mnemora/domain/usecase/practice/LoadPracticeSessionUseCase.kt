...
class LoadPracticeSessionUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val questions = when {
            ...
            filter.isNotBlank() -> applyFilter(allQuestions, filter, navBookId)
            initialNodeId.isNotBlank() -> allQuestions.filter { it.nodeId == initialNodeId }
            else -> allQuestions
        }

        val effectiveBookId = if (collectionId > 0 && questions.isNotEmpty()) {
    ...
    private suspend fun applyFilter(questions: List<Question>, filter: String, bookId: Int): List<Question> {
