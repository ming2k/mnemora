...
class ManageProgressUseCase @Inject constructor(
    private val userAnswerRepository: UserAnswerRepository,
    private val studySessionRepository: StudySessionRepository
) {
    suspend fun saveSessionProgress(sessionId: Long, currentIndex: Int, totalQuestions: Int) {
        if (sessionId > 0) {
            studySessionRepository.updateSessionProgress(sessionId, currentIndex, totalQuestions)
        }
    }

    suspend fun toggleMark(bookId: Int, questionId: Int, isMarked: Boolean) {
    ...
    suspend fun resetAllProgress(bookId: Int, currentSessionId: Long) {
