...
class ManageCollectionUseCase @Inject constructor(
    private val collectionRepository: CollectionRepository
) {
    suspend fun getAvailableCollections(bookId: Int) = collectionRepository.getCustomCollections(bookId)
    
    suspend fun getQuestionCollectionIds(bookId: Int, questionId: Int) = 
        collectionRepository.getCollectionIdsForQuestion(bookId, questionId).toSet()

    suspend fun toggleQuestionInCollection(collectionId: Int, questionId: Int, isIn: Boolean) {
        if (isIn) {
    ...
    suspend fun createCollection(bookId: Int, name: String) {
