...
class SettingsRepositoryTest {
    ...
    @Test
    fun `switching provider model combinations restores their own connection values`() = runBlocking {
        val repository = SettingsRepository(FakeDataStore())
        val solProfile = AiConnectionProfile(
            apiKey = "sk-sol",
        ...
        val newLunaProfile = repository.switchAiConnection(
        ...
        val lunaProfile = AiConnectionProfile(
            apiKey = "sk-luna",
            baseUrl = "https://luna.example.com/v1",
            reasoningEffort = "low",
        )
        val restoredSolProfile = repository.switchAiConnection(
            previousProvider = "custom-openai",
