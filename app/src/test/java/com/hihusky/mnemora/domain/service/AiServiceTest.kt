...
class AiServiceTest {
    ...
    private fun factory() = AiProviderFactory(OkHttpClient())

    /** AiService whose persisted settings never emit, so updateConfig is authoritative. */
    private fun service() = AiService(FakeDataStore(emptyFlow()), factory())
    ...
    @Test
    fun `loads persisted provider and model on startup`() =
        runBlocking {
            val prefs =
                preferencesOf(
            ...
            val aiService = AiService(FakeDataStore(flowOf(prefs)), factory())
    ...
    @Test
    fun `migrates legacy persisted provider to the default`() =
        runBlocking {
    ...
    @Test
    fun `loads connection values from active provider model profile`() =
        runBlocking {
            val profiles =
